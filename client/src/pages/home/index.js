import { useSelector } from "react-redux";
import ChatArea from "./components/chat";
import Header from "./components/header";
import Sidebar from "./components/sidebar";
import { io } from "socket.io-client";
import { useEffect, useState } from "react";

// Initialize socket instance safely
const socket = io("https://chitchat-2-0gd1.onrender.com", {
transports: ["websocket", "polling"]
});

function Home() {
const { selectedChat, user } = useSelector(
state => state.userReducer
);

const [onlineUser, setOnlineUser] = useState([]);

useEffect(() => {
    if (!user) return;

    // Tell the backend who we are and join our room
    socket.emit("join-room", user._id);
    socket.emit("user-login", user._id);

    // Listen for the initial online users state
    socket.on("online-users", (onlineusers) => {
        setOnlineUser(onlineusers);
    });

    // Listen for changes when users disconnect/connect
    socket.on("online-users-updated", (onlineusers) => {
        setOnlineUser(onlineusers);
    });

    // Remove socket listeners when the component unmounts
    return () => {
        socket.off("online-users");
        socket.off("online-users-updated");
    };
}, [user]);

return (
    <div className="home-page">
        <Header socket={socket} />

        <div className="main-content">
            <Sidebar
                socket={socket}
                onlineUser={onlineUser}
            />

            {selectedChat && (
                <ChatArea socket={socket} />
            )}
        </div>
    </div>
);


}

export default Home;
